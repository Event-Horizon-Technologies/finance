...
class AlphaVantage(Static):
    ...
    @staticmethod
    def get_intraday_extended_stock_data(
        symbol, interval, slice="year1month1", adjusted=ADJUSTED, include_metadata=True
    ):
        return get_pandas_dataframe(
    ...
    def get_daily_adjusted_stock_data(
        symbol, outputsize=OUTPUTSIZE, include_metadata=True
    ):
        return get_pandas_dataframe(
    ...
    @staticmethod
    def get_news_and_sentiments_data(
        tickers="",
        topics="",
        time_from="",
        time_to="",
    ...
    def get_stock_data(symbol, interval, include_metadata=True, outputsize=OUTPUTSIZE):
        function = (
            "TIME_SERIES_DAILY"
            if interval == "1day"
            else "TIME_SERIES_INTRADAY_EXTENDED"
        )
