...
class AlphaVantage(Static):
    ...
    @staticmethod
    def get_intraday_stock(symbol, interval, adjusted=ADJUSTED, outputsize=OUTPUTSIZE, datatype=DATATYPE):
    ...
    @staticmethod
    def get_intraday_extended_stock(symbol, interval, slice="year1month1", adjusted=ADJUSTED):
        return pandas_read_csv(
    ...
    @staticmethod
    def get_daily_stock(symbol, outputsize=ADJUSTED, datatype=DATATYPE):
        return pandas_read_csv(
            f"{URL_BASE}function=TIME_SERIES_DAILY&symbol={symbol}&outputsize={outputsize}&datatype={datatype}&apikey={API_KEY}"
        )

    @staticmethod
    def get_daily_adjusted_request(symbol, outputsize=ADJUSTED, datatype=DATATYPE):
        return pandas_read_csv(
    ...
    @staticmethod
    def get_stock_data(symbol, interval, datatype=DATATYPE, outputsize=OUTPUTSIZE):
