...
class BuyAndHold(Strategy):
    ...
    def get_transactions(self, simulator) -> dict[str, float]:
        return (
            {self.symbols[0]: simulator.cash}
            if simulator.now == simulator.start_date
            else {}
        )
...
class MeanReversion(Strategy):
    ...
    def get_transactions(self, simulator) -> dict[str, float]:
        date = simulator.now
        ema_val = simulator.indicator_data[self.symbols[0]][
            self.ind_label
        ].get_val_by_date(date)
...
class PSAR_EMA(Strategy):
    ...
    def __get_indicator_value(self, label) -> float:
        return self.simulator.indicator_data[self.symbols[0]][label].get_val_by_date(
            self.simulator.now
        )

    def __get_asset_value(self) -> float:
        return self.simulator.investments[self.symbols[0]].asset.get_price_by_date(
            self.simulator.now
        )
    ...
    def get_transactions(self, simulator) -> dict[str, float]:
        ...
        if (
            simulator.cash > 0
            and self.__get_indicator_value(self.psar_label) < self.__get_asset_value()
            and self.__get_indicator_value(self.short_period_label)
            > self.__get_indicator_value(self.long_period_label)
        ):
            return {self.symbols[0]: simulator.cash}
