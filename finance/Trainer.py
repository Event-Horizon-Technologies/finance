...
class Trainer:
    def __init__(self) -> None:
        self.input_train = (
            self.target_train
        ) = self.input_test = self.target_test = self.model = None
    ...
    def __standardize(self) -> None:
        self.input_means = np.apply_along_axis(
            np.mean, 0, np.concatenate((self.input_train, self.input_test))
        )
        self.target_mean = np.mean(
            np.concatenate((self.target_train, self.target_test))
        )

        self.input_stds = np.apply_along_axis(
            np.std, 0, np.concatenate((self.input_train, self.input_test))
        )
    ...
    def create_model(
        self, hidden_layers=16, width=32, activation=activations.selu, dropout=0.5
    ) -> None:
        self.model = models.Sequential()

        self.model.add(
            layers.Dense(
                units=width, input_shape=(self.input_size,), activation=activation
            )
        )
    ...
    def generate_data(
        ...
        if not (saved_assets or symbols):
            raise Exception(
                "No assets to generate data with. Must provide either 'symbols' or 'saved_assets'"
            )
        if not indicators:
        ...
        target_test = []
        data_points = sum(
            len(asset.close.values) - prediction_offset for asset in assets
        )
        ...
        for asset in assets:
            ...
            input_data = np.column_stack(
                [i.create_neural_net_data(asset, prediction_offset) for i in indicators]
            )
            target_data = np.log(
                prices[prediction_offset:] / prices[:-prediction_offset]
            )
        ...
        self.input_train, self.target_train = np.concatenate(
            input_train
        ), np.concatenate(target_train)
        self.input_test, self.target_test = np.concatenate(input_test), np.concatenate(
            target_test
        )
    ...
    def train(self, batch_size=sys.maxsize, epochs=sys.maxsize, patience=3) -> None:
        self.model.fit(
            x=self.input_train,
            y=self.target_train,
            validation_data=(self.input_test, self.target_test),
